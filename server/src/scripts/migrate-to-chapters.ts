...
async function main() {
  ...
  try {
    ...
    const { rows: stories } = await client.query<{ id: string; title: string }>(
      'SELECT id, title FROM "Story"'
    );

    for (const story of stories) {
      // Créer un chapitre par défaut
      const { rows: chapters } = await client.query<{ id: string }>(
        `INSERT INTO "Chapter" (id, title, description, "order", "storyId", "createdAt", "updatedAt")
         VALUES (gen_random_uuid()::text, $1, $2, 1, $3, NOW(), NOW())
         RETURNING id`,
        [`Chapitre 1`, `Chapitre importé depuis "${story.title}"`, story.id]
      );
      ...
      try {
        await client.query(
          `UPDATE "Scene" SET "chapterId" = $1 WHERE "storyId" = $2`,
          [chapterId, story.id]
        );
      } catch {

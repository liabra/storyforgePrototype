...
export const update = async (req: Request, res: Response) => {
  ...
  if (req.body.title && !moderateText(req.body.title, "story.title").isAllowed)
    return res.status(400).json({ error: MOD_REFUSED });
  if (req.body.description && !moderateText(req.body.description, "story.description").isAllowed)
    return res.status(400).json({ error: MOD_REFUSED });

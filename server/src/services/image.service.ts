...
type ImageProvider = (ctx: ImageContext) => Promise<string>;
 ...
 */
export function buildVisualPrompt(ctx: ImageContext): string {
  const parts: string[] = [];

  parts.push(
    `Fantasy digital illustration, scene titled "${ctx.sceneTitle}" from the story "${ctx.storyTitle}"`
  );
  ...
  parts.push(
    "cinematic lighting, detailed background, atmospheric, painterly style, high quality"
  );
 ...
 */
const placeholderProvider: ImageProvider = async (ctx) => {
 ...
 */
const cloudflareProvider: ImageProvider = async (ctx) => {
  ...
  if (!accountId || !apiToken) {
    throw new Error(
      "IMAGE_PROVIDER=cloudflare requiert CF_ACCOUNT_ID et CF_API_TOKEN dans .env"
    );
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Cloudflare Workers AI error ${response.status}: ${body}`
    );
  }
 ...
 */
export async function generateImage(ctx: ImageContext): Promise<string> {
  const providerKey = (process.env.IMAGE_PROVIDER ?? "placeholder").toLowerCase();
  const provider = PROVIDERS[providerKey];

  if (!provider) {
    console.warn(
      `[image.service] Provider inconnu "${providerKey}", fallback sur placeholder.`
    );
